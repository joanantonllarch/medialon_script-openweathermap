   ...
   //**************************************************************************
    ...
    _parse_recv: function ( answer ) {
        ...
        {   var gauge;
            ...
            gauge = ( answer.hourly[i].temp - 10 ) * 10;
            if ( gauge >= 1 && gauge < 2  )
                // graphic cheat
                gauge = 2;
            QMedialon.SetValue( "GAG_temp_" + i + ".Status" , gauge );
            // rain probability
            QMedialon.SetValue( "TXT_rain_value_" + i + ".Text" , answer.hourly[i].pop.toFixed(1) );
            gauge = answer.hourly[i].pop * 2;
            if ( gauge >= 1 && gauge < 2  )
                gauge = 2;
            QMedialon.SetValue( "GAG_rain_" + i + ".Status" , gauge );
            // wind speed
            QMedialon.SetValue( "TXT_wind_value_" + i + ".Text" , answer.hourly[i].wind_speed.toFixed(1) );
            gauge = answer.hourly[i].wind_speed * 10;
            if ( gauge >= 1 && gauge < 2  )
                gauge = 2;
            QMedialon.SetValue( "GAG_wind_" + i + ".Status" , gauge );
        }
        // current
